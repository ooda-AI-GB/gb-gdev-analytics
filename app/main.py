...
from app.routes import dashboard, dashboards, sources, widgets, query, insights, billing, api
# Start imports for viv-auth and viv-pay
from viv_auth import init_auth
from viv_pay import init_pay

app = FastAPI(title="Analytics Pro")
...
# Initialize Auth
User, require_auth = init_auth(app, engine, Base, get_db, app_name="Analytics Pro")

# Initialize Pay
create_checkout, get_customer, require_subscription = init_pay(app, engine, Base, get_db, app_name="Analytics Pro")
...
def startup_event():
    ...
    # This includes User (from viv-auth), Billing tables (from viv-pay), and Analytics models
    # We must import app.models so they are registered in Base
    import app.models
